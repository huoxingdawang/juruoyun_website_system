<?php
	...
	try
	{
		...
		else if($_GET['action']=='show')
		{
			...
			$st->execute();
			if((urldecode($_POST["mail_show"])!=2&&($jry_wb_login_user['head']['type']=='gravatar'))||(urldecode($_POST["oauth_show"])==0&&($jry_wb_login_user['head']['type']=='qq'||$jry_wb_login_user['head']['type']=='gitee'||$jry_wb_login_user['head']['type']=='github'||$jry_wb_login_user['head']['type']=='mi')))
			{
				if($jry_wb_login_user['sex']==0&&$jry_wb_login_user['head']!='default_head_woman')
				{
					$q ='update '.JRY_WB_DATABASE_GENERAL.'users set head=\'{"type":"default_head_woman"}\',lasttime=? where id=?';
