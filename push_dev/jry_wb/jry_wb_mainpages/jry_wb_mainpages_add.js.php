...
<?php if(JRY_WB_CHECK_TEL_SWITCH&&JRY_WB_SHORT_MESSAGE_SWITCH!=''){ ?>
	...
	var td			=document.createElement('td')		;tcodetr.appendChild(td)		;td			.classList.add('phonecode')		;td.innerHTML='电话验证码';
	var td			=document.createElement('td')		;tcodetr.appendChild(td)		;td			.classList.add('phonecode_v')	;
...
<?php } ?>
	...
	button.onclick=function()
	{
		...
		<?php if(JRY_WB_CHECK_TEL_SWITCH&&JRY_WB_SHORT_MESSAGE_SWITCH!=''){ ?>phonecode.value='';<?php } ?>
		sexs[0].click();
	...
	tijiao_button.onclick=function()
	{
		...
		jry_wb_ajax_load_data('do_add.php?debug='+jry_wb_get_get().debug,function (data)
		{
			...
			if(data.code)
			{
				jry_wb_beautiful_alert.alert('已注册,第一遍提醒','您的<span class="jry_wb_add_id">ID='+data.id+'</span><br>ID是您登录本网站的<span class="jry_wb_add_id">唯一</span>凭证<br>请牢记',function()
				{
					jry_wb_cache.delete('add');
					jry_wb_beautiful_alert.alert('已注册,第二遍提醒','您的<span class="jry_wb_add_id">ID='+data.id+'</span><br>ID是您登录本网站的<span class="jry_wb_add_id">唯一</span>凭证<br>请牢记',function()
					{
						jry_wb_beautiful_alert.alert('已注册,第三遍提醒','您的<span class="jry_wb_add_id">ID='+data.id+'</span><br>ID是您登录本网站的<span class="jry_wb_add_id">唯一</span>凭证<br>请牢记',function()
						{
			...
			else
			{
				...
				<?php if(JRY_WB_CHECK_TEL_SWITCH&&JRY_WB_SHORT_MESSAGE_SWITCH!=''){ ?>	
					...
					jry_wb_beautiful_alert.alert("注册失败","手机验证码错误",function(){phonecode.focus();phonecode.classList.add('error');});	
				<?php } ?>
	...
	<?php if(JRY_WB_CHECK_TEL_SWITCH&&JRY_WB_SHORT_MESSAGE_SWITCH!=''){ ?>
		if(e.target==tijiao_button&&phonecode.value=='')return jry_wb_beautiful_alert.alert("请填写完整信息","电话验证码为空",function(){phonecode.focus();phonecode.classList.add('error');}),false;
	<?php } ?>
