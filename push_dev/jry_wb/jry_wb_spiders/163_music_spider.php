...
//热评API http://music.163.com/api/v1/resource/comments/R_SO_4_574566207
	...
	function jry_wb_163_music_spider($in_mid)
	{
		...
		$st = $conn->prepare('SELECT * FROM '.JRY_WB_DATABASE_SPIDERS.'163_music where mid=?');
		$st->bindParam(1,$in_mid);
		$st->execute();
		$data=$st->fetchAll();
		if(count($data)!=0&&(strtotime(jry_wb_get_time())-strtotime($data[0]['lasttime']))>60*60*0.25)
		{
			$ch=curl_init('http://music.163.com/weapi/song/enhance/player/url?csrf_token=');
			...
			$st = $conn->prepare('UPDATE '.JRY_WB_DATABASE_SPIDERS.'163_music SET `music_url`=?,`lasttime`=? WHERE mid=?');
		...
		if(count($data)==0)
		{
			...
			$ch=curl_init('https://music.163.com/weapi/cloudsearch/get/web?csrf_token=');
			curl_setopt($ch,CURLOPT_HEADER, 0);    
			curl_setopt($ch,CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);    
			curl_setopt($ch,CURLOPT_RETURNTRANSFER, 1); 
			curl_setopt($ch,CURLOPT_FOLLOWLOCATION, 1);
			...
			$ch=curl_init('http://music.163.com/api/song/media?id='.$in_mid);
			curl_setopt($ch,CURLOPT_HEADER, 0);    
			curl_setopt($ch,CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);    
			curl_setopt($ch,CURLOPT_RETURNTRANSFER, 1); 
			curl_setopt($ch,CURLOPT_FOLLOWLOCATION, 1);
			...
			curl_close($ch);	
			$get_sorce=json_decode($get_sorce);
			$get_data['lyric']=$get_sorce->lyric;
			$st = $conn->prepare('INSERT INTO '.JRY_WB_DATABASE_SPIDERS.'163_music (`mid`,`pic_url`,`name`,`album`,`music_url`,`singers`,`lyric`,`lasttime`) VALUES (?,?,?,?,?,?,?,?)');
			...
			$st->bindParam(7,$get_data['lyric']);
			$st->bindParam(8,$get_data['lasttime']=jry_wb_get_time());
			$st->execute();
			$get_data['type']='163';
		}
		else
			$get_data=array('mid'=>$data[0]['mid'],'type'=>'163','pic_url'=>$data[0]['pic_url'],'name'=>$data[0]['name'],'album'=>$data[0]['album'],'music_url'=>$data[0]['music_url'],'singers'=>$data[0]['singers'],'lasttime'=>$data[0]['lasttime'],'lyric'=>$data[0]['lyric']);
		return ($get_data);
	}
