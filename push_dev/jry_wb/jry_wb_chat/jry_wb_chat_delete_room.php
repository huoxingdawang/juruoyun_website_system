<?php
	...
	function jry_wb_chat_delete_room($conn,&$user,&$roomm)
	{
		...
		$st = $conn->prepare('UPDATE '.JRY_WB_DATABASE_CHEAT.'rooms SET `delete`=1,last_say_time="1926-08-17 00:00:00",lasttime=? WHERE  chat_room_id=?');
		$st->bindValue(1,jry_wb_get_time());
		$st->bindValue(2,$room['chat_room_id']);
		$st->execute();
		$st = $conn->prepare('DELETE FROM '.JRY_WB_DATABASE_CHEAT.'message WHERE  chat_room_id=?');
		$st->bindValue(1,$room['chat_room_id']);
		$st->execute();
		$st = $conn->prepare('UPDATE '.JRY_WB_DATABASE_CHEAT.'users SET lasttime=?,chat_rooms=JSON_REMOVE(chat_rooms,JSON_UNQUOTE(JSON_SEARCH(chat_rooms,"one",?))) WHERE  JSON_CONTAINS(chat_rooms,?)');
		$st->bindValue(1,jry_wb_get_time());
