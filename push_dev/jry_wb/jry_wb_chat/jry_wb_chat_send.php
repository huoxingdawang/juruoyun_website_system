<?php
	...
	function jry_wb_chat_send($conn,$sender,&$roomm,$message)
	{
		...
		jry_wb_check_compentence($sender,['usechat'],NULL);	
		...
		$st = $conn->prepare('INSERT INTO '.JRY_WB_DATABASE_CHAT.'message (id,chat_room_id,message,send_time) VALUES (?,?,?,?)');
		...
		$st = $conn->prepare('UPDATE '.JRY_WB_DATABASE_CHAT.'rooms SET say_count=say_count+1 , last_say_time=?,lasttime=? WHERE chat_room_id=?');
		$st->bindValue(1,jry_wb_get_time());
		$st->bindValue(2,jry_wb_get_time());
		$st->bindValue(3,$room['chat_room_id']);
		$st->execute();
		$st = $conn->prepare('UPDATE '.JRY_WB_DATABASE_CHAT.'users SET  say_count=say_count+1, lasttime=? WHERE id=?');
		$st->bindValue(1,jry_wb_get_time());
