			...
			switch(data[i][j])
			{
				...
				case '!':
					...
					if((url[0]=='h'&&url[1]=='t'&&url[2]=='t'&&url[3]=='p')||url[0]=='.'||url[0]=='/')
						...
						img.src='http://juruoyun.top/mywork/picturebed/get_picturebed.php?pictureid='+url+'&size='+parseInt(this.area.clientWidth);
				...
				case '-':
					...
					if(data[i][j]=='['&&(((data[i][j+1]=='x'||data[i][j+1]=='X')&&data[i][j+2]==']')||data[i][j+1]==']'))
					{
						...
						{ 
							var b=document.createElement("b");text.appendChild(b);b.classList.add('md_finish','iconfont','icon-duigoux');
							j+=2;
						}
						else
						{
							var b=document.createElement("b");text.appendChild(b);b.classList.add('md_unfinish','iconfont','icon-hr');							
							j+=1;
						}
					}
					else
