<?php
	...
	if($action=='list')
	{
		...
		$st->execute();
		$st = $conn2->prepare('SELECT `id`,`name`,`use`,`lasttime` FROM '.JRY_WB_DATABASE_GENERAL.'users WHERE `lasttime`>? AND `order`>?');
		$st->bindValue(1,urldecode($_GET['lasttime']));
		$st->bindValue(2,$jry_wb_login_user['order']);
		$st->execute();
		$json=array();
		foreach($st->fetchAll()as $users)
			$json[]=array('id'=>(int)$users['id'],'use'=>$users['use'],'name'=>$users['name'],'lasttime'=>$users['lasttime']);
