...
/*	if($_GET['action']=='aly_download')
	{
		...
		/*$st = $conn->prepare('INSERT INTO '.JRY_WB_DATABASE_NETDISK.'test (`data`,`lasttime`) VALUES (?,?)');
		$st->bindValue(1,json_encode($receive_data));
		$st->bindValue(2,jry_wb_get_time());
		$st->execute();
		
		$st = $conn->prepare('SELECT * FROM '.JRY_WB_DATABASE_NETDISK.'area WHERE `type`=1 AND config_message->"$.bucket"=? AND `use`=1');
		...
		$st = $conn->prepare('SELECT * FROM '.JRY_WB_DATABASE_NETDISK.'file_list WHERE `area`=? AND file_id=? AND use=1');
