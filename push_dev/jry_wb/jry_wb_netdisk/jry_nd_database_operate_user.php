<?php
	...
	{
		$st = $conn->prepare('UPDATE '.constant('jry_wb_netdisk').'users SET size_used=? , size_uploading=? , lasttime=? WHERE `id`=?;');
		$st->bindValue(1,$used);
		$st->bindValue(2,$uploading);
		$st->bindValue(3,jry_wb_get_time());
		$st->bindValue(4,$user['id']);
	...
	{
		$st = $conn->prepare('UPDATE '.constant('jry_wb_netdisk').'users SET size_total=size_total+?,lasttime=? WHERE `id`=?;');
