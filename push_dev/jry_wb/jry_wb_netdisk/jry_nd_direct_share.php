<?php
	...
	function jry_nd_direct_share($conn,$user,$file)
	{
		...
		{
			if($file['share_list']!='')
				$st = $conn->prepare('UPDATE '.constant('jry_wb_netdisk').'file_list SET `share`=?,`lasttime`=?,`share_list`=JSON_MERGE_PRESERVE(share_list,?),self_share=self_share|? WHERE `file_id`=? AND id=? LIMIT 1');
			else
				$st = $conn->prepare('UPDATE '.constant('jry_wb_netdisk').'file_list SET `share`=?,`lasttime`=?,`share_list`=?,self_share=self_share|? WHERE `file_id`=? AND id=? LIMIT 1');
			$st->bindValue(1,1);
			...
			if($file['isdir']==1)
			{
				if($file['self_share']==1&&$file['isdir']==1)
				{
					...
					$st = $conn->prepare('UPDATE '.constant('jry_wb_netdisk').'file_list SET `share`=?,`lasttime`=?,`share_list`=JSON_MERGE_PRESERVE(share_list,?) WHERE id=? AND file_id!=? AND JSON_CONTAINS(share_list,?)');
					$st->bindValue(1,1);
					$st->bindValue(2,jry_wb_get_time());
					$st->bindValue(3,json_encode(array((string)$share_id)));
	...
	function jry_nd_direct_unshare($conn,$user,$file)
	{
		...
		{
			$st = $conn->prepare('UPDATE '.constant('jry_wb_netdisk').'file_list SET `lasttime`=? , share_list=JSON_REMOVE(share_list,JSON_UNQUOTE(JSON_SEARCH(share_list,\'one\',?))) WHERE JSON_CONTAINS(share_list,?);');
