<?php
	...
	function jry_nd_direct_share($conn,$user,$file)
	{
		...
		$st = $conn->prepare('INSERT INTO '.constant('jry_wb_database_netdisk').'share (`id`,`key`,`file_id`,`lasttime`) VALUES (?,?,?,?)');
		...
		{
			if($file['share_list']!='')
				$st = $conn->prepare('UPDATE '.constant('jry_wb_database_netdisk').'file_list SET `share`=?,`lasttime`=?,`share_list`=JSON_MERGE_PRESERVE(share_list,?),self_share=self_share|? WHERE `file_id`=? AND id=? LIMIT 1');
			else
				$st = $conn->prepare('UPDATE '.constant('jry_wb_database_netdisk').'file_list SET `share`=?,`lasttime`=?,`share_list`=?,self_share=self_share|? WHERE `file_id`=? AND id=? LIMIT 1');
			$st->bindValue(1,1);
			...
			if($file['isdir']==1)
			{
				if($file['self_share']==1&&$file['isdir']==1)
				{
					...
					$st = $conn->prepare('UPDATE '.constant('jry_wb_database_netdisk').'file_list SET `share`=?,`lasttime`=?,`share_list`=JSON_MERGE_PRESERVE(share_list,?) WHERE id=? AND file_id!=? AND JSON_CONTAINS(share_list,?)');
					$st->bindValue(1,1);
					$st->bindValue(2,jry_wb_get_time());
					$st->bindValue(3,json_encode(array((string)$share_id)));
	...
	function jry_nd_direct_unshare($conn,$user,$file)
	{
		if($file['share']==0)
			return;
		$st = $conn->prepare('SELECT share_id FROM '.constant('jry_wb_database_netdisk').'share WHERE `file_id`=? AND id=?');
		$st->bindValue(1,$file['file_id']);
		$st->bindValue(2,$user['id']);			
		$st->execute();
		foreach($st->fetchAll() as $one)
		{
			$st = $conn->prepare('UPDATE '.constant('jry_wb_database_netdisk').'file_list SET `lasttime`=? , share_list=JSON_REMOVE(share_list,JSON_UNQUOTE(JSON_SEARCH(share_list,\'one\',?))) WHERE JSON_CONTAINS(share_list,?);');
			$st->bindValue(1,jry_wb_get_time());
			$st->bindValue(2,(string)$one['share_id']);
			$st->bindValue(3,json_encode(array((string)$one['share_id'])));
			$st->execute();
		}
		$st = $conn->prepare('UPDATE '.constant('jry_wb_database_netdisk').'file_list SET self_share=0 WHERE `file_id`=? AND id=?');
		$st->bindValue(1,$file['file_id']);
		$st->bindValue(2,$user['id']);			
		$st->execute();
		...
		$st->execute();
		$file['share']=0;
		$st = $conn->prepare('DELETE FROM '.constant('jry_wb_database_netdisk').'share WHERE  id=? AND file_id=?');
		$st->bindValue(1,$user['id']);
