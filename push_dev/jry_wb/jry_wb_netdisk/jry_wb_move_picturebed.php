...
//	print_r($area);
	...
	foreach($data as $photo)
	{
		...
		$st = $conn->prepare('INSERT INTO '.constant('jry_wb_netdisk').'file_list (`id`,`dir`,`name`,`type`,`area`,`size`,`lasttime`,`uploading`) VALUES (?,?,?,?,?,?,?,?)');
		...
		fclose($src);		
		fclose($dst);		
		$st = $conn->prepare('INSERT INTO '.constant('jry_wb_netdisk').'share (`id`,`key`,`file_id`,`lasttime`,`share_id`) VALUES (?,?,?,?,?)');
		$st->bindValue(1,$photo['id']);
		$st->bindValue(2,'');
