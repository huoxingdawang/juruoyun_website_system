<?php
	...
	function jry_nd_database_operate_fast_save($mode,$time)
	{
		if(($file=fopen('jry_nd.fast_save_message','r'))==false)
		{
			...
			if($mode=='group')
			{
				$st = $conn->prepare('SELECT lasttime FROM '.JRY_WB_DATABASE_NETDISK.'area ORDER BY lasttime DESC LIMIT 1;');
				$st->execute();	
			...
			{
				$st = $conn->prepare('SELECT lasttime FROM '.JRY_WB_DATABASE_NETDISK.'group ORDER BY lasttime DESC LIMIT 1;');
				$st->execute();	
