			...
			if($socket===$master)
			{
				...
				else
				{
					...
					if($ip=='127.0.0.1')
					{
						...
						echo jry_wb_php_cli_color($data['user']['id'].'-'.$data['user']['name'],'light_blue')."\t".jry_wb_php_cli_color('get ','green').jry_wb_php_cli_color(strlen($data['data']),'magenta').'/B data '.substr($data['data'],0,100)."\n";
						$data['data']=json_decode($data['data']);
						try
						{
							call_back($data['data'],$data['user']);
						}
						catch (jry_wb_exception $e)
						{
							$error=json_decode($e->getMessage());
							jry_wb_socket_send($socket,$error);
							echo jry_wb_php_cli_color('Failed!','light_red').' At FILE:'.jry_wb_php_cli_color($error->file,'yellow').' LINE:'.jry_wb_php_cli_color($error->line,'yellow').' Because '.jry_wb_php_cli_color($error->reason,'magenta')."\n";
						}
						socket_close($client);
					}
					else
