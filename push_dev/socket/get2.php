...
//	$test_machine_queue=new array();\
	...
	echo "Oh new test machine connection!\nnumber:".$count."\nIP:".$remoteIP." , Port:".$remotePort."\n";
	
	$socket_for_web = socket_create(AF_INET,SOCK_STREAM,SOL_TCP);
	if(socket_bind($socket_for_web,constant("test_machine_jry_wb_host_ip"),constant("test_machine_port_for_web")) == false)
	{
		echo "server bind fail:".socket_strerror(socket_last_error())."\n";
		socket_close($socket_for_web);
		exit();
	}
	echo "Bind OK\r\n";
	if(socket_listen($socket_for_web,4)==false)
	{
        echo "server listen fail:".socket_strerror(socket_last_error())."\n";
		socket_close($socket_for_web);
		exit();
	...
	do
	{
		$accept_resource_for_web = socket_accept($socket_for_web);
		socket_getpeername($accept_resource_for_web, $remoteIP, $remotePort);
		echo "Oh new web connection!\nnumber:".$count."\nIP:".$remoteIP." , Port:".$remotePort."\n";		
		if(socket_read($accept_resource_for_web,4096)!='test new')
		{
			echo "Error\n\n";
			continue;
		}
		echo "A new task is created\n";
		$count++;
		...
		$message=json_encode($callback);
		socket_write($accept_resource_for_web,$message,strlen($message));
		socket_close($accept_resource_for_web);
	}while(true);
	echo "exit\n";
